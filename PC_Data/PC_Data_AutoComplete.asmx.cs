...
namespace ZCZJ_DPF.PC_Data
{
    ...
    public class PC_Data_AutoComplete : System.Web.Services.WebService
    {
        ...
        public String[] GetTask(string prefixText, int count)
        {
            ...
            if (autoCompleteWordList == null)
            {
                ...
                sqlConn.Open();
                DataTable glotb = new DataTable();
                glotb = DBCallCommon.GetDTUsingSqlText("select Expr1 from (SELECT distinct  ISNULL(b.CM_CONTR,'') + '|' + ISNULL(a.TSA_ID,'')+ '|' + ISNULL(b.CM_PROJ,'')  as Expr1 FROM TBCM_Basic a, TBCM_Plan b WHERE a.ID=b.ID and a.TSA_ID LIKE '%" + prefixText + "%')t union (select distinct ISNULL(a.TSA_PJID,'') + '|' + ISNULL(a.TSA_ID,'')+ '|' + ISNULL(b.CM_PROJ,'')  as Expr1  from TBPM_TCTSASSGN as a left join TBCM_Plan as b on a.TSA_PJID=b.CM_CONTR where TSA_ID like '%" + prefixText + "%')");
                glotb.AcceptChanges();
                //读取内容文件的数据到临时数组
                string[] temp = new string[glotb.Rows.Count];
                int i = 0;
