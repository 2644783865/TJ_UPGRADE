...
var superTable = function (tableId) {
    ...
    for (i=0;i<this.rowCount;i++)
    {
       if(!this.rowHeights[i]||this.rowHeights[i]===-1)
       {
         ...
         this.rowHeights[i] = this.sourceTable.tBodies[0].rows[i].offsetHeight;
          
//            var maxheight=this.sourceTable.tBodies[0].rows[i].cells[0].offsetHeight;
//          
//            for (j=0; j<this.columnCount; j++)
//            {
//               if(this.sourceTable.tBodies[0].rows[i].cells[j].offsetHeight>maxheight)
//               {
//                 maxheight=this.sourceTable.tBodies[0].rows[i].cells[j].offsetHeight;
//               }
//            }
//            
//            this.rowHeights[i]=maxheight;
        ...
        for (i=(this.sourceTable.tHead) ? 0 : this.headerRows; i<this.tBodyRowCount; i++) {
            ...
            for (j=0; j<this.fixedCols; j++) {
                try {
                    ...
                    dest.cells[j].style.height = dest.style.height;
    ...
    //css
    for (i=0, j=(this.columnCount - this.fixedCols); i<j; i++) {
    //只需要设置某一行列的宽度，其他的都相同，故rows[0]的索引为0
	...
	this.sFHeader.className = "sFHeader" + this.skin;//左边固定表头
	this.sHeader.className = "sHeader" + this.skin;//右边表头
	this.sFData.className = "sFData" + this.skin;//左边固定数据
	this.sData.className = "sData" + this.skin;//右边数据
...
//         else { alert("IE"); }
        if (/*@cc_on!@*/0) { /* Internet Explorer */
			window.attachEvent("onunload", function () {
				superTables[x].sData.onscroll = null;
				superTables[x] = null;
//				alert("zsw1！");
			});
		}
...
//			<%=ClientScript.GetPostBackEventReference(Repeater1, "") %>;
//				alert("zsw")

//alert(this.sourceTable.parentNode.id);
////              superTables[x] = null;
