<%@ Page Language="C#" MasterPageFile="~/Masters/NoScollRightCotentMaster.Master"
    AutoEventWireup="true" CodeBehind="ys_task_budget_change.aspx.cs" Inherits="ZCZJ_DPF.YS_Data.UI.ys_task_budget_change"
    Title="" %>
        ...
        <asp:Panel ID="pal_container" runat="server">
            <div style="height: 350px;">
                <table align="center" width="100%" cellpadding="4" cellspacing="1" border="1" class="toptable grid">
                    <asp:Repeater ID="rpt_task_list" runat="server" OnItemDataBound="rpt_task_list_databound"  OnItemCommand="Repeater1_ItemCommand" >
                        <HeaderTemplate>
                            <tr class="tableTitle">
                                <th>
                                    序号
                            ...
                            <tr align="center" class="baseGadget" id="row" runat="Server" onmouseover="this.className='highlight'"
                                ...
                                <td>
                                    <%#Eval("task_code").ToString()%>
                                     <asp:HiddenField ID="HiddenField1" runat="server" Value='<%# Eval("task_code") %>' />
                                </td>
                                <td>
                                ...
                                <td>
                                <asp:LinkButton ID="Button1" runat="server" CssClass="link" OnClientClick="javascript:return confirm('是否要更新预算编制?')" CommandArgument='<%#Eval("task_code") %>'  CommandName="change" >
                                <asp:Image ID="Image4" runat="server" ImageUrl="~/Assets/icons/dealdispatcher.gif" ImageAlign="AbsMiddle"></asp:Image> 更新</asp:LinkButton> 
                                 <%-- <asp:Button ID="Button1" runat="server" Text="更 新" />--%>
                               <%-- <asp:HyperLink ID="HyperLink1" NavigateUrl='<%#"ys_task_budget_detail.aspx?tsak_code="+Eval("task_code").ToString()%>'
                                ...
                                <td>
                                    <%#Eval("change_time").ToString()%>
                                </td>
                                <td>
                                    <%#Eval("change_name").ToString()%>
                                </td>
                            </tr>
                        </ItemTemplate>
                    </asp:Repeater>
