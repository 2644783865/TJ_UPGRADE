<%@ Page Language="C#" MasterPageFile="~/Masters/RightCotentMaster.Master" AutoEventWireup="true" CodeBehind="OM_SALARYBASEDATAJLCK.aspx.cs" Inherits="ZCZJ_DPF.OM_Data.OM_SALARYBASEDATAJLCK" Title="薪酬基数记录查看" %>
                                ...
                                <asp:Repeater ID="rptGDGZrecord" runat="server">
                                    ...
                                    <ItemTemplate>
                                        <tr class="baseGadget" onmouseover="this.className='highlight'" onclick="javascript:change(this);"
                                            ...
                                            <td>
                                                <asp:Label ID="BASEDATANEW" runat="server" BorderStyle="None" BackColor="Transparent"
                                                    Width="50px" Text='<%#Eval("BASEDATANEW")%>'></asp:Label>
                                            </td>
                                            <td>
                                                <asp:Label ID="BASEDATAOLD" runat="server" BorderStyle="None" BackColor="Transparent"
                                                    Width="50px" Text='<%#Eval("BASEDATAOLD")%>'></asp:Label>
                                            </td>
                                            <td>
                                                <asp:Label ID="CZRST_NAME" runat="server" Text='<%#Eval("CZRST_NAME")%>'></asp:Label>
                                            </td>
                                            <td>
                                                <asp:Label ID="CZTIME" runat="server" Text='<%#Eval("CZTIME")%>'></asp:Label>
                                            </td>
                                            <td>
                                                <asp:TextBox ID="NOTE" runat="server" Width="150px" Text='<%#Eval("NOTE")%>'
                                                    BorderStyle="None" Style="background-color: Transparent; text-align: center"
                                                    ToolTip='<%#Eval("NOTE")%>'></asp:TextBox>
                                            </td>
                                            <td>
                                                <asp:Label ID="TYPE" runat="server" Text='<%#Eval("TYPE").ToString().Trim()=="BINGJIA"?"病假工资基数":Eval("TYPE").ToString().Trim()=="JIABAN"?"加班工资基数":Eval("TYPE").ToString().Trim()=="NIANJIA"?"年假工资基数":"应扣岗位基数"%>'></asp:Label>
                                            </td>
                                        </tr>
                                    </ItemTemplate>
                                </asp:Repeater>
